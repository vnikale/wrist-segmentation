    ...
    def attention_block(input_enc, input_dec, n_filt):
        ...
        if config.AT_CONV_TYPE == 'depth':
            conv11 = DepthwiseConv2D(1, activation=None, padding='valid', kernel_initializer='he_normal',
                                     strides=(2, 2,2), depth_multiplier=2, name=conv_name + '11')(input_enc)
        ...
        # conv11 = DepthwiseConv2D(1, activation=None, padding='valid', kernel_initializer='he_normal', strides=(2,2), depth_multiplier=2, name=conv_name + '11')(input_enc)
        # conv12 = DepthwiseConv2D(1, activation=None, padding='valid', kernel_initializer='he_normal', name=conv_name + '12')(input_dec)
        ...
        if config.AT_CONV_TYPE == 'depth':
            conv2 = DepthwiseConv2D(1, activation='sigmoid', padding='valid', kernel_initializer='he_normal')(relu)
        else:
            conv2 = Conv2D(1, 1, activation='sigmoid', padding='valid', kernel_initializer='he_normal',
                           name=conv_name + '2')(relu)


        up2 = UpSampling2D(size=2)(conv2)
        up2_exp = expend_as(up2, n_filt)
        mult = Multiply(name=mult_name)([input_enc, up2_exp])

        if config.AT_CONV_TYPE == 'depth':
            conv3 = DepthwiseConv2D(1, activation='sigmoid', padding='valid', kernel_initializer='he_normal',
                                    name=conv_name + '3')(mult)
        else:
