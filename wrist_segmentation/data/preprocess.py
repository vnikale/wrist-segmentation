...
def crop_preprocess(image, size):
    im = rotate(image)
    im = center_crop(im, size)
    im = white_norm_img(im)

    ma = rotate(image)
    ma = center_crop(im, size)
    return im, ma
...
def preprocess_wrapper(inputs):
    ...
    :param args:
    :return: image, mask or only image depends on preprocess_func
    '''
    assert 'image'in inputs.keys() or 'mask'in inputs.keys(), 'There are no images or masks in args dict'
